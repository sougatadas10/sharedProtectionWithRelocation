...
//Step5: Discard the reversed of edges of path1 & path2. Interchange the remaining edges to give 2 edge disjoint path
        ...
        if (flag){
        	//tempEdgePath1=new ArrayList<Edge>();
        	tempEdgePath1=new ArrayList<Edge>(edgePath1);
        	//for (int k=0;k<edgePath1.size();++k){
        	//	tEdge=new Edge(edgePath1.get(k).getId(),edgePath1.get(k).getSource(),
        	//			edgePath1.get(k).getDestination(),edgePath1.get(k).getWeight());
        	//	tempEdgePath1.add(tEdge);
        	//	++j;
        	//}
        	...
        	for (int i=index1;i<tempEdgePath1.size();++i){
        		System.out.println("i="+i+"\t"+edgePath1.size());
